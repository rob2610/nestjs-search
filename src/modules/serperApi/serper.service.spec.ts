...
describe('SerperApiService', () => {
  ...
  describe('ImageResponse', () => {
    it('Should return serper images', async () => {
      const response = {
        images: [{
          title: 'Cagnocervo',
          imageUrl: 'cagnocervo.jpg',
  ...
  describe('getImages', () => {
    it('Should fetch and process images', async () => {
      ...
      jest.spyOn(service as any, 'buildUrl').mockReturnValue('https://test.com');
      jest.spyOn(service as any, 'imagesRequest').mockResolvedValue(response);
      jest.spyOn(service as any, 'imageResponse').mockReturnValue(response.data.images);
